...
def main(args):
    ...
    P1_filtered = P1_filtered[distances_P1<args.distance_threshold]
    P2_filtered = P2_filtered[distances_P2 < args.distance_threshold]

    ####################################

    plt.hist(distances_P2, bins=100)
    plt.show()
    plt.hist(distances_P1, bins=100)
    plt.show()
...
########################################3
    ...
    #P1_trans_est =  np.dot(P1 , R_est.T) + t_est
    #P1_trans_est = P1
    P1_trans_gt =  np.dot( P1, R_gt.T) +  t_gt  #scale * np.dot( (P1 - mu1), np.linalg.inv(R_gt).T) + mu2


    P1_filtered_trans_est = scale_est * np.dot(P1_filtered, R_est.T) + t_est
    colors1_filtered = np.tile( np.array( [[1, 0, 0]] ), (len(P1_filtered_trans_est), 1)  )
    colors2_filtered = np.tile( np.array( [[0, 1, 0]] ), (len(P2_filtered), 1)  )

    #colors1_filtered = (1, 0, 0)*len(P1_filtered_trans_est)
    #colors2_filtered = (0, 1, 0) * len(P1_filtered_trans_est)

    points_filtered_est = np.vstack((P1_filtered_trans_est, P2_filtered))
    colors_filtered = np.vstack((colors1_filtered, colors2_filtered))

    ################################
    pcd_filtered_est = o3d.geometry.PointCloud()
    pcd_filtered_est.points = o3d.utility.Vector3dVector(points_filtered_est)
    pcd_filtered_est.colors = o3d.utility.Vector3dVector(colors_filtered)
    ################################
    ...
    choose_start_pose(pcd_filtered_est, args)

    choose_start_pose(pcd_est, args)

    choose_start_pose(pcd_gt, args)
